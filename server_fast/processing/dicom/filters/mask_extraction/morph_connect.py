...
class MorphConnectFilter(BaseFilter):
    def apply(self, img: np.ndarray, kernel_size: int = 5, iterations: int = 1, **kwargs) -> np.ndarray:
        """ 
        Conecta áreas cercanas (Operación Closing morfológico).
        """
        ...
        if is_binary:
            ...
            result = binary_closing(img, structure=structure, iterations=iterations)
        else:
            structure_num = np.ones((kernel_size, kernel_size))
            result = img.copy()
            for _ in range(iterations):
                result = grey_closing(result, footprint=structure_num)
